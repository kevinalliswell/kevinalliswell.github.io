...
用一个更硬的类比：这就是控制论里的**反馈控制回路**。传统程序是**开环控制**——按下按钮，走完预设流程，不管结果对不对。Agent 是**闭环控制**——它持续测量「当前状态与目标的偏差」，并用行动去缩小这个偏差。搞自动化和控制的工程师对这个闭环会心一笑：这不就是 PID 吗？只不过把「比例-积分-微分」这个固定的控制律，换成了一个**会推理的、能处理非结构化反馈的 LLM 控制器**。
...
![传统软件 vs Agent 开发：十维对比](imgs/03.jpg)
...
![Agent 的思考循环：Think→Plan→Act→Observe→Reflect](imgs/05.jpg)

### 逐环拆解：一个真实的 debug 场景

抽象的循环不好体会，我们用一个「Agent 修复失败测试」的真实过程走一遍。假设目标是「让 `test_payment.py` 全部通过」，Agent 会这样转几圈：

1. **Act**：调用 `run_tests()`；**Observe**：`3 passed, 1 failed`，AssertionError 在第 42 行。
2. **Reflect**：断言在 42 行挂了；**Plan**：去读那一段实现。
3. **Act**：`read_file("payment.py", 30, 50)`；**Observe**：拿到源码，发现四舍五入用错了函数。
4. **Act**：`edit_file(...)` 改这一行。**关键**：不能相信自己改对了，必须再验证。
5. **Act**：再次 `run_tests()`；**Observe**：`4 passed` 全绿；**Reflect**：目标达成，退出循环。

看懂这个过程，你就看懂了 Claude Code、Cursor、Devin 在底层做的事情——它们不是「一次生成正确代码」，而是**「生成 → 用真实反馈验证 → 发现不对 → 改 → 再验证」的循环**。这也解释了一个常见困惑：为什么 Agent 有时会「兜圈子」？因为它在做真实的探索，而探索本就不是一条直线。
...
![Agent 技术栈：九个概念如何分层协同](imgs/06.jpg)
...
![IDE vs Agent：方向盘 vs 司机](imgs/07.jpg)
...
![每个行业的 Agent：同一骨架的九次实例化](imgs/08.jpg)
...
这里最重要的，是那个「转人工」的出口。**一个设计良好的 Agent，不是想方设法自己扛下一切，而是清楚地知道自己能力和权限的边界，并在越界前主动把方向盘交回给人。** 「知道什么时候该求助」，是自主性里最成熟的一种，也是最容易被新手忽略的一种。
...
![软件架构的重排：从调用栈到智能核 + 能力总线](imgs/10.jpg)

### 旧骨架：确定性的分层调用栈

它的世界观是：**功能是预先定义的，用户在预设的功能集里做选择。** 数据确定性地流过每一层。过去三十年的软件，几乎都是这种分层调用栈的变体。它可靠、可预测，但它的能力边界，在编译期就被焊死了。
